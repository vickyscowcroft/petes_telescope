...
def find_ref_star():
    current_image = PR.camera_routines.image_to_array()
    star_locations = PR.camera_routines.find_star_locations(current_image)
...
def centralise(reference_star):
    ...
    if altitude_movement == 0:
        ...
        while altitude_movement < 0:
            PR.stepper_routines.altMotor.singleForward(ALT_STEPS_PER_PIXEL*altitude_movement)
            reference_star = find_ref_star()
            altitude_movement = (PIXEL_LENGTH/2) - reference_star[1]
    elif altitude_movement > 0:
        while altitude_movement < 0:
            PR.stepper_routines.altMotor.stepper.singleBackward(ALT_STEPS_PER_PIXEL*altitude_movement)
            reference_star = find_ref_star()
            altitude_movement = (PIXEL_LENGTH/2) - reference_star[1]
    ...
    if azimuth_movement == 0:
        ...
        while azimuth_movement < 0:
            PR.stepper_routines.azMotor.singleForward(ALT_STEPS_PER_PIXEL*azimuth_movement)
            reference_star = find_ref_star()
            azimuth_movement = (PIXEL_LENGTH/2) - reference_star[1]
    elif azimuth_movement > 0:
        while azimuth_movement < 0:
            PR.stepper_routines.azMotor.singleBackward(ALT_STEPS_PER_PIXEL*azimuth_movement)
            reference_star = find_ref_star()
            azimuth_movement = (PIXEL_LENGTH/2) - reference_star[1]
